...
class Network:
    def __init__(self, dataset, config):
        ...
        if config.resume:
            checkpoint_path = join(config.resume_path, 'snapshots')
            latest_checkpoint = tf.train.latest_checkpoint(checkpoint_path)
            if latest_checkpoint:
                print(f"Resuming training from checkpoint: {latest_checkpoint}")
                self.saver.restore(self.sess, latest_checkpoint)
                # Extract the global step from the checkpoint name
                # Checkpoint names are like "snap-12345", so we get the number after the dash
                self.training_step = int(latest_checkpoint.split('-')[-1])
                # Estimate the epoch based on steps per epoch
                steps_per_epoch = config.train_steps
                self.training_epoch = self.training_step // steps_per_epoch
                print(f"Resuming from step {self.training_step}, estimated epoch {self.training_epoch}")
            else:
