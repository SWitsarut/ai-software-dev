...
class Toronto3D:
    def __init__(self, inputs_dir,mode='test'):
        self.name = 'Toronto3D'
        # self.path = 'data/Toronto_3D'
        self.input_path = inputs_dir
        ...
        full_paths = [os.path.join(self.input_path, file) for file in files]


        ply_paths = [path for path in full_paths if path.endswith('.ply')]

        # ply_names_no_ext = [os.path.splitext(os.path.basename(path))[0] for path in full_paths if path.endswith('.ply')]

        self.test_files = ply_paths
        ...
        self.load_sub_sampled_clouds(mode)
        cfg.saving = False
        self.init_test_pipeline()
        print('start making model')
        model = Network(self, cfg)

        print('start making tester')
        chosen_snap = "./results/Log_2025-04-02_18-20-48/snapshots/snap-9501"
        tester = ModelTester(model, self, cfg, restore_snap=chosen_snap)
        tester.test(model, self)


    def load_sub_sampled_clouds(self, mode):
        ...
        for i, file_path in enumerate(files):
            ...
            kd_tree_file = join(self.input_path, '{:s}_KDTree.pkl'.format(cloud_name))
            sub_ply_file = join(self.input_path, '{:s}.ply'.format(cloud_name))
            ...
            if cloud_split == 'test':
                ...
                proj_file = join(self.input_path, '{:s}_proj.pkl'.format(cloud_name))
